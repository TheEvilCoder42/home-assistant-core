...
async def async_setup_entry(hass: HomeAssistant, entry: DenonavrConfigEntry) -> bool:
    ...
    def _propagate_connectivity_to_settings() -> None:
        ...
        means the settings data can't be trusted either, regardless of
        its own polling schedule. A recovery only propagates when the
        settings coordinator has no recurring poll of its own
        ...
        an actual settings refresh confirming it.
        """
        if (
            coordinator.last_update_success
            and settings_coordinator.update_interval is not None
        ):
            return
    ...
    entry.async_on_unload(
        coordinator.async_add_listener(_propagate_connectivity_to_settings)
    )
    ...
    def _propagate_settings_failure_to_general() -> None:
        ...
        the settings coordinator's own polling schedule: a
        settings-backed select/switch action failing with a
        connectivity error means the receiver itself is unreachable,
        exactly like a media_player command failing the same way. Only
        mirrors failure, never recovery - the status coordinator's own
        poll is what should confirm it's back.
        """
        if not settings_coordinator.last_update_success:
            mark_unavailable(coordinator)

    entry.async_on_unload(
        settings_coordinator.async_add_listener(_propagate_settings_failure_to_general)
    )

    # These settings aren't populated by regular status queries, so
    ...
    if use_telnet and update_audyssey:
        ...
        # it only pushes these settings on a change, never on connect,
    ...
    def _telnet_notify_settings(zone: str, event: str, parameter: str) -> None:
        """Notify settings listeners of Telnet activity.
        ...
        regardless, so settings entities need a path independent of it.
        """
        settings_coordinator.async_update_listeners()
